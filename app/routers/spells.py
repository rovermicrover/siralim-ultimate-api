...
def index(session = Depends(has_session)):
  spells_orm = SpellOrm.get_all(session, options=EAGER_LOAD_OPTIONS)
  spells_model = SpellModel.from_orm_list(spells_orm)
  return IndexSchema(data=spells_model)
...
def get(spell_id: str, session = Depends(has_session)):
  spell_orm = SpellOrm.get_by_slug_or_id(session, spell_id, options=EAGER_LOAD_OPTIONS)
