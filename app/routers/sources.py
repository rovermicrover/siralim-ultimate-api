...
def index(session = Depends(has_session), pagination: PaginationSchema = Depends(pagination_depend), sorting: SortingSchema = Depends(sorting_depend)):
  sources_orm = select(SourceOrm).pagination(pagination).sorting(sorting).get_scalars(session)
...
def get(source_id: str, session = Depends(has_session)):
  source_orm = select(SourceOrm).where(SourceOrm.where_slug_or_id(source_id)).get_scalar(session)
