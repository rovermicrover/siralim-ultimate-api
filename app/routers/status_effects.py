...
def index(session = Depends(has_session), pagination: PaginationSchema = Depends(pagination_depend), sorting: SortingSchema = Depends(sorting_depend)):
  status_effects_orm = select(StatusEffectOrm).pagination(pagination).sorting(sorting).get_scalars(session)
...
def get(status_effect_id: str, session = Depends(has_session)):
  status_effect_orm = select(StatusEffectOrm).where(StatusEffectOrm.where_slug_or_id(status_effect_id)).get_scalar(session)
