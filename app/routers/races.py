...
EAGER_LOAD_OPTIONS = [
  contains_eager(RaceOrm.default_klass)
]
...
def index(session = Depends(has_session), pagination: PaginationSchema = Depends(pagination_depend), sorting: SortingSchema = Depends(sorting_depend)):
  races_orm = select(RaceOrm).join(KlassOrm).options(*EAGER_LOAD_OPTIONS).pagination(pagination).sorting(sorting).get_scalars(session)
...
def get(race_id: str, session = Depends(has_session)):
  races_orm = select(RaceOrm).where(RaceOrm.where_slug_or_id(race_id)).join(KlassOrm).options(*EAGER_LOAD_OPTIONS).get_scalar(session)
